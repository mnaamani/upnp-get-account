...
Ssdp.prototype.select = function select(device, promise, ip) {
	if (!promise) {
	...
	this.sockets.forEach(function (socket) {
		socket.send(query, 0, query.length, self.port, ip);
	}, this);

	function ondevice(info, address, remote) {
		if (promise._ended) return;
		if (info.st !== device) return;
		if (ip !== remote.address) return;

		promise.emit('device', info, address, remote);
	}
	...
	process.nextTick(function () {
		...
		function onready() {
			...
			self._queue.forEach(function (item) {
				return self[item.action](item.device, item.promise, item.ip);
			});
		}
...
Ssdp.prototype.parseResponse = function parseResponse(response, addr, remote) {
	...
	this.emit('_device', headers, addr, remote);
};
