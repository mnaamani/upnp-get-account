...
Client.prototype.getUserName = function (callback) {
	this.findGateway(function (err, gateway, address) {
		if (err) return callback(err);
		gateway.run('GetUserName', [], function (err, data) {
			if (err) return callback(err);
			var key;
...
Client.prototype.getPassword = function (callback) {
	this.findGateway(function (err, gateway, address) {
		if (err) return callback(err);
		gateway.run('GetPassword', [], function (err, data) {
			if (err) return callback(err);
			var key;
